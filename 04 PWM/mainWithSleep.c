...
int main(void) {
	...
	while(1){
		...
		//Sleep mode
		_BIS_SR(LPM4);  //Low-power mode 4. CPU y relojes deshabilitados
						//_BIS_SR => Bit Set de Special Functions Register (SR)
	}
...
__interrupt void P2_INT(void){
	if(P2IFG & 0x02){ 				//Si la bandera de P2.1 se encuentra activa, P2.1 caus'o la interrupcion
		...
		_BIC_SR(LPM4_EXIT);			//Salir del modo de bajo consumo para aplicar nuevo dutyCycle
									//_BIC_SR => Bit Clear de Special Functions Register (SR)
	}
}
